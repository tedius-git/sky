...
function intFromUnalignedSliceUsingNumber(buffer, start4, end, isBigEndian, isSigned) {
  ...
  if (isBigEndian) {
    ...
    if (isStartByteAligned) {
      ...
      let scale2 = 1;
      while (size3 >= 8) {
        value += buffer[byteIndex++] * scale2;
        scale2 *= 256;
        size3 -= 8;
      }
      value += (buffer[byteIndex] >> 8 - size3) * scale2;
    } else {
  ...
  #createAttribute(node, attribute3) {
    ...
    switch (attribute3.kind) {
      ...
      case event_kind: {
        ...
        const stop2 = attribute3.stop_propagation;
        ...
        nodeMeta.handlers.set(attribute3.name, (event4) => {
          ...
          if (stop2) event4.stopPropagation();
          ...
          while (pathNode !== this.#root) {
            ...
            if (key) {
              ...
              path2 = `${separator_index}${index3}${path2}`;
            }
            pathNode = parent;
          }
          path2 = path2.slice(1);
...
function dot(v, u) {
  return fold(map2(v, u, multiply), 0, add);
}
function equals(v, u, tol) {
  let is_close = map2(
    v,
    u,
    (x2, y2) => {
      return loosely_equals(x2, y2, tol);
    }
  );
  return all(is_close, (x2) => {
    return x2;
  });
}
function mod(v) {
  return fold(map(v, (x2) => {
    return x2 * x2;
  }), 0, add);
}
...
function to_string4(v) {
  return "[" + (() => {
    let _pipe = fold(
      map(v, float_to_string),
      "",
      (a2, b) => {
        return a2 + "," + b;
      }
...
function sum_forces(particle, all2) {
  let r = particle.r;
  let others = filter(
    all2,
    (p) => {
...
function new_particle(r, v, m) {
  ...
  let x2 = r.head;
  let y2 = r.tail.head;
  return new Particle(m, toList([x2, y2]), v, toList([0, 0]));
}
...
function setup_mouse_down_listener(dispatch) {
  const handleMouseDown = (event4) => {
    const rect = event4.target.getBoundingClientRect();
    const x2 = event4.clientX - rect.left;
    const y2 = event4.clientY - rect.top;
    dispatch(x2, y2);
...
function setup_mouse_up_listener(dispatch) {
  const handleMouseUp = (event4) => {
    const rect = event4.target.getBoundingClientRect();
    const x2 = event4.clientX - rect.left;
    const y2 = event4.clientY - rect.top;
    dispatch(x2, y2);
...
var Model = class extends CustomType {
  constructor(debug, light_on, width, height, paused, particles, time, timer_id, mouse, mouse_down_pos) {
    ...
    this.mouse = mouse;
    this.mouse_down_pos = mouse_down_pos;
  }
};
...
var MouseDown = class extends CustomType {
  constructor(x0, x1) {
    super();
    this[0] = x0;
    this[1] = x1;
...
var MouseUp = class extends CustomType {
  constructor(x0, x1) {
    super();
    this[0] = x0;
    this[1] = x1;
  ...
  return from(
    (dispatch) => {
      let timer_id = set_interval(
        10,
        () => {
          return dispatch(new IncreseTime());
        }
      );
      return dispatch(new TimerStarted(timer_id));
    }
  );
}
...
function update_particles_effect(delta) {
  return from(
    (dispatch) => {
      return dispatch(new UpdateParticles(delta));
    }
  );
}
function setup_mouse_tracking() {
  return from(
    (dispatch) => {
      ...
      let $1 = setup_mouse_down_listener(
        (x2, y2) => {
          return dispatch(new MouseDown(x2, y2));
        }
      );
      let $2 = setup_mouse_up_listener(
        (x2, y2) => {
          return dispatch(new MouseUp(x2, y2));
        }
      );
      return void 0;
    }
...
function update2(model, msg) {
  ...
  let time = model.time;
  if (msg instanceof UserToggleTheme) {
    return [
      (() => {
        let _record = model;
        return new Model(
  ...
  } else if (msg instanceof UserTogglePaused) {
    let $ = model.timer_id;
    if (paused) {
      return [
        (() => {
    ...
    } else if (!paused && $ instanceof Some) {
      let timer_id = $[0];
      return [
        (() => {
          let _record = model;
  ...
  } else if (msg instanceof TimerStarted) {
    let timer_id = msg[0];
    return [
      (() => {
        let _record = model;
  ...
  } else if (msg instanceof IncreseTime) {
    return [
      (() => {
        let _record = model;
        return new Model(
  ...
  } else if (msg instanceof UserIncreseTime) {
    return [
      (() => {
        let _record = model;
        return new Model(
  ...
  } else if (msg instanceof UserDecreseTime) {
    let $ = time === 0;
    if ($) {
      return [model, none()];
    } else {
      ...
      (() => {
        ...
        return new Model(
          ...
          (() => {
            ...
            return first(partition(particles));
          })(),
          _record.time,
          _record.timer_id,
          _record.mouse,
  ...
  } else if (msg instanceof MouseMoved) {
    let x2 = msg[0];
    let y2 = msg[1];
    return [
      (() => {
    ...
    let x2 = msg[0];
    let y2 = msg[1];
    let $ = model.mouse_down_pos;
    if ($ instanceof None) {
      return [model, none()];
    } else {
      let start_pos = $[0];
      if (!start_pos.hasLength(2)) {
        throw makeError(
          "let_assert",
          "app",
          197,
      ...
      let dx = x2 - x_0;
      let dy = y2 - y_0;
      let velocity = toList([divideFloat(dx, 10), divideFloat(dy, 10)]);
      echo(
        "p_0 " + to_string4(toList([x_0, y_0])),
        "src/app.gleam",
        201
      );
      echo("p " + to_string4(toList([x2, y2])), "src/app.gleam", 202);
      echo("v " + to_string4(velocity), "src/app.gleam", 203);
      ...
      return [
        (() => {
          ...
          return new Model(
            ...
            _record.height,
            _record.paused,
            append(model.particles, toList([new_p])),
            _record.time,
            _record.timer_id,
            _record.mouse,
            new None()
...
function echo(value, file, line2) {
  const grey = "\x1B[90m";
...
function view_button_text(handle_click, text4) {
  return button(
    toList([on_click(handle_click)]),
    toList([text3(text4)])
  );
}
function div_glass(attributes, elements) {
  return div(
    append(
      attributes,
      toList([
        ...
        class$("flex flex-col m-4 p-3")
      ])
    ),
    elements
  );
}
function view_toggle_theme(light_on) {
  let size2 = class$("size-6");
      ...
      (() => {
        if (paused) {
          return div(
            ...
            toList([
              view_button_text(new UserDecreseTime(), "-"),
              view_button_text(new UserIncreseTime(), "+")
            ])
          );
        } else {
    ...
    linear_gradient(
      ...
      toList([
        stop(
          toList([
            attribute2("offset", "0%"),
            attribute2("stop-color", "#f36364")
          ])
        ),
        stop(
          toList([
            attribute2("offset", "50%"),
            attribute2("stop-color", "#f36364")
          ])
        ),
        stop(
          toList([
            attribute2("offset", "100%"),
            attribute2("stop-color", "#f292ed")
          ])
        )
      ])
    ),
    linear_gradient(
      ...
      toList([
        stop(
          toList([
            attribute2("offset", "0%"),
            attribute2("stop-color", "#c6f8ff")
          ])
        ),
        stop(
          toList([
            attribute2("offset", "50%"),
            attribute2("stop-color", "#595cff")
          ])
        ),
        stop(
          toList([
    ...
    linear_gradient(
      ...
      toList([
        stop(
          toList([
            attribute2("offset", "0%"),
            attribute2("stop-color", "#f3f520")
          ])
        ),
        stop(
          toList([
            attribute2("offset", "50%"),
            attribute2("stop-color", "#59d102")
          ])
        ),
        stop(
          toList([
...
function view_sim(model) {
  return svg(
    ...
    (() => {
      ...
      let _pipe$2 = append(_pipe$1, launch_v);
      return append(
        _pipe$2,
        toList([
          defs(
            toList([]),
            append(view_colors(), view_arrow_marker())
          )
        ])
      );
    })()
...
function view(model) {
  ...
  let $ = model.light_on;
  if ($) {
    _block = class$("bg-white");
  } else {
    _block = class$("bg-black");
  }
  let bg = _block;
  let _block$1;
  let $1 = model.light_on;
  if ($1) {
    _block$1 = class$("text-black");
  } else {
